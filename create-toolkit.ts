...
/**
 * create-toolkit - Scaffold a monorepo toolkit with SDK, CLI (Stricli), and MCP (FastMCP)
 *
 * Usage:
 *   bun run create-toolkit.ts <name>
 *   bun run create-toolkit.ts twilio
 *   bun run create-toolkit.ts cloudflare --description "Cloudflare management tools"
 *
...
if (values.help || positionals.length === 0) {
  console.log(`
  create-toolkit - Scaffold a monorepo toolkit (SDK + CLI + MCP)

  Usage:
    bun run create-toolkit.ts <name> [options]
  ...
  Examples:
    bun run create-toolkit.ts twilio
    bun run create-toolkit.ts cloudflare -d "Cloudflare management tools"
  `);
  process.exit(0);
}
...
const rootPackageJson = `{
  ...
  "scripts": {
    ...
    "dev:mcp": "bun run --filter '${scope}/mcp' dev",
    "dev:cli": "bun run --filter '${scope}/cli' dev",
...
A monorepo containing the SDK, CLI, and MCP server for the ${namePascal} API.
...
const sdkTest = `import { describe, expect, it } from "bun:test";
